...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(async () => {
    ...
    boot = (async () => {
      try {
        const update = await updater.checkAndInstall();
        if (update.installing) return;
      } catch {
        // Se fuerza una comprobación en cada arranque, pero una caída temporal
        // del servicio de actualizaciones no impide usar el launcher.
      }
