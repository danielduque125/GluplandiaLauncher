      ...
      {tab === 'equipo' && <section className="settings tab-scene" key="equipo">
        ...
        <div className="settings-grid-two">
          ...
          <div className="settings-card">
            ...
            <p>Versión instalada <strong>{data?.launcherVersion || '—'}</strong>. Las actualizaciones automáticas se activan en la edición de distribución firmada.</p>
            <button disabled={busy} onClick={() => action(async () => { const result = await api.update(); if (!result.enabled) setEvent({ phase: 'update-disabled', message: 'Las actualizaciones automáticas se habilitan en el instalador de distribución firmado.' }); })}>Comprobar actualización</button>
          </div>
        </div>
      </section>}
